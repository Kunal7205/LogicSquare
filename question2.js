function findCaliforniaCafes(searchTerm) {
    ...
    for (const cafe of cafes) {
      const mPlaces = places.filter(place => place.id === cafe.place_id);
  
      for (const mPlace of mPlaces) {
        if (cafe.name.toLowerCase().includes(searchTerm.toLowerCase())) {
          results.push({
            name: cafe.name,
            street_no: mPlace.street_no,
            locality: mPlace.locality,
            postal_code: mPlace.postal_code,
            lat: mPlace.lat,
            long: mPlace.long
          });
        }
  ...
  const results = findCaliforniaCafes('Bazaar');
  console.log(results);